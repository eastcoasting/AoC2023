...
const b = () => {
  ...
  let potentialSeedsInput: string[] = [];
  ...
  text.split("\n").map((line, idx) => {
    if (idx === 0) {
      ...
      if (potentialSeeds) {
        ...
        if (seedInputData) {
          potentialSeedsInput = seedInputData;
        }
      }
    }
    if (line === "") {
  ...
  for (let idx = 0; idx < potentialSeedsInput.length; idx++) {
    const currentValue = potentialSeedsInput[idx];

    if (idx % 2) {
      // these are the range
      const seedRange = +currentValue;
      const seedStart = +potentialSeedsInput[idx - 1];
      ...
      for (let rangeIdx = 0; rangeIdx < seedRange; rangeIdx++) {
        ...
        currentValue = checkInRange(
          lookupMaps["seed-to-soil"],
          newSeedRangeItem
        );
        currentValue = checkInRange(
          lookupMaps["soil-to-fertilizer"],
          currentValue
        );
        currentValue = checkInRange(
